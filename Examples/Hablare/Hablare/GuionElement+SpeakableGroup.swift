...
public struct SceneGroup: SpeakableGroup {
  ...
  public func getGroupedElements() -> [any SpeakableItem] {
    ...
    let nextSceneIndex =
      document.elements[(sceneIndex + 1)...].firstIndex { element in
...
public struct SectionGroup: SpeakableGroup {
  ...
  public func getGroupedElements() -> [any SpeakableItem] {
    ...
    guard let sectionIndex = document.elements.firstIndex(where: { $0.id == sectionHeading.id })
    else {
      return []
    }
    ...
    let nextSectionIndex =
      document.elements[(sectionIndex + 1)...].firstIndex { element in
