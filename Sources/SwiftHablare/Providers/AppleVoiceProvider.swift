...
public final class AppleVoiceProvider: VoiceProvider {
  ...
  public func estimateDuration(text: String, voiceId: String) async -> TimeInterval {
    let request = engine.makeRequest(
      text: text, voiceId: voiceId, languageCode: LanguageCodeResolver.systemLanguageCode)
  ...
  public func generateProcessedAudio(text: String, voiceId: String, languageCode: String)
  ...
  private struct AppleVoiceProviderConfigurationView: View {
    ...
    var body: some View {
      VStack(alignment: .leading, spacing: 16) {
        ...
        Text(
          "This provider uses on-device speech synthesis and does not require additional configuration."
        )
        .font(.callout)
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Toggle(isOn: $filterHighQualityOnly) {
            VStack(alignment: .leading, spacing: 4) {
              ...
              Text(
                "Filters out Standard Quality voices to show only higher quality options. Note: Some languages may have limited high-quality voices available."
              )
              .font(.caption)
              .foregroundStyle(.secondary)
            }
