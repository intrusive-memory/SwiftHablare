...
final class AVSpeechTTSEngine: AppleTTSEngine {
  ...
  func generateAudioWithDuration(text: String, voiceId: String, languageCode: String) async throws
    -> (Data, TimeInterval)
  {
    // Validate text is not empty
    guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
  ...
  func fetchVoices(languageCode: String) async throws -> [Voice] {
    ...
    return try await MainActor.run {
      ...
      #if DEBUG
        print(
          "🎤 [AVSpeechTTSEngine] Returned from AVSpeechSynthesisVoice.speechVoices() with \(avVoices.count) voices"
        )
      #endif
      ...
      let allVoices = avVoices.compactMap { avVoice -> Voice? in
        ...
        let languageInfo =
          Locale.current.localizedString(forIdentifier: avVoice.language) ?? avVoice.language
      ...
      Task { @MainActor in
        do {
          ...
          synthesizer.write(utterance) { buffer in
            #if DEBUG
              print(
                "🎤 [AVSpeechTTSEngine] ✅ Buffer callback invoked! Buffer type: \(type(of: buffer))")
            #endif
            ...
            do {
              ...
              if audioFile == nil {
                ...
                #if DEBUG
                  print(
                    "🎤 [AVSpeechTTSEngine] ✅ Created audio file with 16-bit PCM at \(sampleRate) Hz"
                  )
                #endif
              }
              ...
              if let file = audioFile {
                ...
                #if DEBUG
                  print(
                    "🎤 [AVSpeechTTSEngine] ✅ Wrote buffer #\(bufferCount) (\(converted.frameLength) frames)"
                  )
                #endif
              }
            } catch {
          ...
          for await event in delegate.events {
            ...
            case .finished, .cancelled:
              ...
              #if DEBUG
                print(
                  "🎤 [AVSpeechTTSEngine] Synthesis complete. Buffer count: \(bufferCount), Total frames: \(totalFrames)"
                )
              #endif
              ...
              #if DEBUG
                print(
                  "🎤 [AVSpeechTTSEngine] ✅ Calculated duration: \(String(format: "%.2f", duration))s from \(totalFrames) frames at \(sampleRate) Hz"
                )
              #endif
              ...
              guard data.count > 1024 else {
                throw VoiceProviderError.networkError(
                  "Generated audio is too short (\(data.count) bytes)")
              }
          ...
          continuation.resume(
            throwing: VoiceProviderError.networkError(
              "Audio generation failed: \(error.localizedDescription)"))
        }
      }
    }
  }
        ...
        await MainActor.run {
          do {
            ...
            let audioFile = try AVAudioFile(
              forWriting: tempURL, settings: settings, commonFormat: .pcmFormatInt16,
              interleaved: false)
            ...
            #if DEBUG
              print(
                "⚠️  Generated placeholder silent audio (\(data.count) bytes, duration: \(String(format: "%.2f", estimatedDuration))s)"
              )
  ...
  private func extractGender(from name: String, identifier: String) -> String? {
    ...
    let maleIndicators = [
      "alex", "daniel", "diego", "fred", "jorge", "juan", "luca", "magnus", "marvin", "nicky",
      "thomas", "yuri",
    ]
    let femaleIndicators = [
      "allison", "ava", "bella", "fiona", "joana", "karen", "kate", "laura", "lekha", "melina",
      "moira", "nora", "paulina", "samantha", "sara", "tessa", "veena", "victoria", "yelda", "zoe",
      "zosia",
    ]
