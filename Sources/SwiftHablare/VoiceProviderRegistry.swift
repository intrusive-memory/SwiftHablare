...
public struct VoiceProviderDescriptor: Identifiable, @unchecked Sendable {
  ...
  #if canImport(SwiftUI)
    public typealias ConfigurationPanelBuilder =
      @MainActor @Sendable (_ provider: VoiceProvider, _ onConfigured: @escaping (Bool) -> Void) ->
      AnyView
  #endif
...
public actor VoiceProviderRegistry {
  ...
  public func contains(providerId: String) -> Bool {
    ...
    @MainActor
    public func configurationPanel(
      for providerId: String, onConfigured: @escaping @MainActor (Bool) -> Void
    ) async -> AnyView? {
      ...
      return descriptor.configurationPanel(provider) { [weak self] success in
        ...
        Task {
          await self.handleConfigurationResult(
            for: descriptor, provider: provider, success: success)
          await MainActor.run {
            onConfigured(success)
          }
        }
  ...
  private static func ensureDefaultStateSync(
    for descriptor: VoiceProviderDescriptor, in userDefaults: UserDefaults
  ) {
  ...
  private func handleConfigurationResult(
    for descriptor: VoiceProviderDescriptor, provider: VoiceProvider, success: Bool
  ) async {
