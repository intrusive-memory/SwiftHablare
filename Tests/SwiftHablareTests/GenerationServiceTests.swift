...
struct GenerationServiceTests {
  ...
  func testActorIsolationEnsuresThreadSafety() async throws {
    ...
    async let result1 = service.generate(
      text: "Test 1", providerId: "apple", voiceId: firstVoice.id)
    async let result2 = service.generate(
      text: "Test 2", providerId: "apple", voiceId: firstVoice.id)
